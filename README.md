...
- **책이지 패스**(사전 위임) 시연: 도서관·기기·범위(관내 무료 상호대차만·월 3권·등록된 기기)를 한 번 정해 두면,
  그 뒤로는 말 한마디로 접수까지 끝난다 — 범위 안 신청은 확인 시트를 건너뛰고 바로 접수, 결과는 매번 통지,
  접수 상태에서는 즉시 취소, 해지는 언제든. 범위 밖(책바다 실비·한도 초과)은 전처럼 확인 한 번을 거친다.
  이달의 추천은 AI 책큐의 일이고 패스와는 별개다. **위임 범위를 화면뿐 아니라 음성으로도 고지한다.**
